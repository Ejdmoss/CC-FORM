...
app.post('/savedata', urlencodedParser, function(req, res) {
  let data = `"${req.body.ukol}","${req.body.cisloKreditKarty}","${req.body.DatumExpirace}","${req.body.cvc}"\n`;
...
app.get('/karty', function(req, res) {
  csvtojson({headers:['ukol','cisloKreditKarty','DatumExpirace','cvc']}).fromFile(path.join(__dirname, 'data/ukoly.csv')) 
  .then( data => {
    console.log(data);
    res.render('index', {nadpis: 'KREDITNÍ KARTY', ukoly: data});
  })
  .catch( error => {
    console.log(error);
  });
